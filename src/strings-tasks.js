...
function removeFirstOccurrences(str, value) {
  ...
  const substr = str.slice(valueIndex + value.length, str.length);
  return str.slice(0, valueIndex) + substr;
}
...
function isPalindrome(str) {
  ...
  const reversedStr = str
    ...
    .toLowerCase()
    .replaceAll(regexp, '');
  const modifiedStr = str.replaceAll(regexp, '').toLowerCase();
